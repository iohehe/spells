...
class Sf_examp : public SouffleProgram {
private:
...
SignalHandler::instance()->setMsg(R"_(path(x,y) :- 
   edge(x,y).
in file /Users/he/spells/DataLog/tutorial/example/example.dl [9:1-9:26])_");
if(!(rel_3_edge->empty())) {
[&](){
...
SignalHandler::instance()->setMsg(R"_(path(x,y) :- 
   path(x,z),
   edge(z,y).
in file /Users/he/spells/DataLog/tutorial/example/example.dl [14:1-14:38])_");
...
SouffleProgram *newInstance_examp(){return new Sf_examp;}
SymbolTable *getST_examp(SouffleProgram *p){return &reinterpret_cast<Sf_examp*>(p)->symTable;}

#ifdef __EMBEDDED_SOUFFLE__
class factory_Sf_examp: public souffle::ProgramFactory {
SouffleProgram *newInstance() {
return new Sf_examp();
};
public:
factory_Sf_examp() : ProgramFactory("examp"){}
};
extern "C" {
factory_Sf_examp __factory_Sf_examp_instance;
}
}
#else
}
